...
public class TeleBlue extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            // G1 triggers: manual transfer motor power (auto power applied during active
            // transfer)
            ...
            telemetry.addData("Transfer",
                    Sorter.isTransferActive() ? "SHOOTING" : Sorter.isTransferPending() ? "PENDING" : "IDLE");
    ...
    private void updateBallTracking() {
        ...
        if (intakeToggled && !shootingMode && !Sorter.isTransferActive() && !manualSpinUsed && sorterAtPosition
                && intakeActive) {
    ...
    private void rotateForShooting() {
        ...
        if (patternShootEnabled && detectedBalls == 3) {
            ...
            for (int i = 0; i < 3; i++) {
                if ("P".equals(ballSlots[i]))
                    pCount++;
                if ("G".equals(ballSlots[i]))
                    gCount++;
            }
