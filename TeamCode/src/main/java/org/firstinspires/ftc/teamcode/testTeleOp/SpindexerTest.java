...
public class SpindexerTest extends LinearOpMode {
    ...
    public static class ServoConfig {
        ...
        // Servo slew speed in servo-units per SECOND (time-based, loop-rate
        // independent).
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (dpadDown && !lastDpadDown && !shootActive) {
                targetPosition = clamp(targetPosition - ServoConfig.stepSize);
                currentSlot = ((currentSlot - 1) % ServoConfig.totalSlots + ServoConfig.totalSlots)
                        % ServoConfig.totalSlots;
            }
            ...
            if (!shootActive) {
                ...
                if (gamepad1.right_trigger > 0.05)
                    manual += gamepad1.right_trigger * ServoConfig.manualSpeed * dt;
                if (gamepad1.left_trigger > 0.05)
                    manual -= gamepad1.left_trigger * ServoConfig.manualSpeed * dt;
                if (manual != 0) {
            ...
            telemetry.addData("Servo2 pos",
                    String.format("%.4f  (%s)", servo2Pos, ServoConfig.servo2Mirrored ? "mirrored" : "same"));
